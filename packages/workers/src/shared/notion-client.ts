 ...
 *
 * RECOVERED SOURCE — reconstructed from dist/shared/notion-client.js (the
 * second-generation build whose TypeScript was never committed). Type
 * annotations restored; runtime behaviour is byte-for-byte equivalent.
 * See docs/ADR/0005-orphan-dist.md.
 */
...
export function getNtnApiToken(): string {
  const token = process.env.NTN_API_TOKEN;
  if (!token) throw new Error("NTN_API_TOKEN is not set");
  return token;
}
 ...
 */
export function resolveTargetDatabaseId(target: TargetDatabase): string {
  switch (target) {
    case "home_docs":
...
export function getGitHubItemsSyncDatabaseId(): string {
  const id = process.env.GITHUB_ITEMS_SYNC_DATABASE_ID;
  if (!id) throw new Error("GITHUB_ITEMS_SYNC_DATABASE_ID is not set");
  return id;
}
...
export function getReferenceDocsDatabaseId(): string | null {
  return process.env.REFERENCE_DOCS_DATABASE_ID || null;
}

export function getAgentSkillsDatabaseId(): string | null {
  return process.env.AGENT_SKILLS_DATABASE_ID || null;
}

export function getSetupTemplatesDatabaseId(): string | null {
...
export function getPlansDatabaseId(): string {
  const id = process.env.PLANS_DATABASE_ID;
  if (!id) throw new Error("PLANS_DATABASE_ID is not set");
  return id;
}
 ...
 */
export function getSubmissionsDataSourceId(): string {
  const id = process.env.SUBMISSIONS_DATA_SOURCE_ID;
  if (!id) throw new Error("SUBMISSIONS_DATA_SOURCE_ID is not set");
  return id;
}
 ...
 * "Plans".
 */
export function getPlansDataSourceId(): string {
  const id = process.env.PLANS_DATA_SOURCE_ID;
  if (!id) throw new Error("PLANS_DATA_SOURCE_ID is not set");
  return id;
}
...
export function getZohoRefreshToken(): string {
  ...
  if (!v) throw new Error("ZOHO_REFRESH_TOKEN is not set");
  return v;
}
...
export function getZohoProjectsPortalId(): string {
  const id = process.env.ZOHO_PROJECTS_PORTAL_ID;
  if (!id) throw new Error("ZOHO_PROJECTS_PORTAL_ID is not set");
  return id;
}

/* ── Data-source resolution & pagination ───────────────────────────── */

/** Minimal shape of the parts of a database response we rely on. */
...
export async function resolveDataSourceId(
  ...
  if (!first) {
    throw new Error(
      `Could not resolve ${inputId} to a data_source — neither a data_source ID nor a parent database with data_sources`
    );
  }
...
/** Query params accepted by queryDatabase, minus the data_source_id we inject. */
...
export async function queryDatabase(
  ...
  if (res.request_status?.type === "incomplete") {
    console.warn(
      `[queryDatabase] Result truncated for database ${databaseId}: ${
        res.request_status.incomplete_reason ?? "unknown reason"
      }. Use queryAllDatabase to fetch all pages.`
    );
  }
  return res;
}
