 ...
 *
 * RECOVERED SOURCE — reconstructed from dist/shared/zoho-client.js.
 ...
 */
export function getZohoAccountsBaseUrl(apiBaseUrl: string): string {
  let hostname: string;
  try {
  ...
  const hostMappings: ReadonlyArray<readonly [string, string]> = [
...
async function fetchAccessToken(): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(
      `Zoho token exchange failed: HTTP ${res.status} ${res.statusText}`
    );
  }
 ...
 */
export async function zohoGet<T = unknown>(path: string): Promise<T> {
  ...
  if (!res.ok) {
    throw new Error(
      `Zoho CRM GET ${path} failed: HTTP ${res.status} ${res.statusText}`
    );
  }
 ...
 */
export async function zohoPatch(
  path: string,
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Zoho CRM PUT ${path} failed: HTTP ${res.status} ${res.statusText} — ${text}`
    );
  }
}
 ...
 */
function getZohoProjectsApiBase(): string {
  return process.env.ZOHO_PROJECTS_API_BASE_URL ?? "https://projectsapi.zoho.com/restapi";
}

export async function zohoProjectsGet<T = unknown>(path: string): Promise<T> {
  const token = await getZohoAccessToken();
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(
      `Zoho Projects GET ${path} failed: HTTP ${res.status} — ${text}`
    );
  }
  return (await res.json()) as T;
}

export async function zohoProjectsPost<T = unknown>(
  path: string,
  body: unknown
): Promise<T> {
  const token = await getZohoAccessToken();
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(
      `Zoho Projects POST ${path} failed: HTTP ${res.status} — ${text}`
    );
  }
  return (await res.json()) as T;
}

export async function zohoProjectsPut<T = unknown>(
  path: string,
  body: unknown
): Promise<T> {
  const token = await getZohoAccessToken();
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(
      `Zoho Projects PUT ${path} failed: HTTP ${res.status} — ${text}`
    );
  }
  return (await res.json()) as T;
}
 ...
 */
export async function zohoCoql<T = Record<string, unknown>>(
  query: string
): Promise<T[]> {
  const token = await getZohoAccessToken();
  const base = getZohoApiBaseUrl();
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(
      `Zoho COQL failed: HTTP ${res.status} ${res.statusText} — ${text}`
    );
  }
