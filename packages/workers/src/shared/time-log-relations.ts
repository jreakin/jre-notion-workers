...
const GH_SRC = { client: "Client", project: "Project", task: "Task" } as const;
...
/** A Notion page's `properties` bag; values are narrowed by the readers below. */
type NotionPropertyBag = Record<string, unknown>;

/** The part of a Notion relation property value this module reads. */
interface RelationPropertyValue {
  relation?: Array<{ id: string }> | null;
}

/** The part of a Notion title property value this module reads. */
interface TitlePropertyValue {
  title?: Array<{ plain_text?: string | null }> | null;
}

/** The part of a Notion unique_id property value this module reads. */
interface UniqueIdPropertyValue {
  unique_id?: { prefix?: string | null; number?: number | null } | null;
}

/** A relation property payload in a page-update request. */
type RelationUpdate = { relation: Array<{ id: string }> };

/** Relations resolved for a single Time Log entry during a backfill pass. */
...
function readRelationIds(
  properties: NotionPropertyBag | undefined,
  propName: string
): string[] {
...
function readTitle(properties: NotionPropertyBag | undefined): string {
...
function readUniqueId(
  ...
  if (!p || typeof p !== "object" || !("unique_id" in p)) return null;
  const uid = (p as UniqueIdPropertyValue).unique_id;
...
export function parseAdIdTokens(title: string): string[] {
  const out: string[] = [];
  for (const m of title.matchAll(AD_TOKEN_RE)) out.push(m[1]!);
  return out;
}

/** Lookup maps built from the Clients and Projects databases. */
export interface UniqueIdMaps {
  /** `[AD-CLIENT-N]` token → Clients page id. */
  clientByUid: Map<string, string>;
  /** `[AD-PROJ-N]` token → Projects page id. */
  projectByUid: Map<string, string>;
  /** Projects page id → the page ids of its Client relation. */
  clientByProjectPageId: Map<string, string[]>;
}
 ...
 */
export async function loadUniqueIdMaps(notion: Client): Promise<UniqueIdMaps> {
  const clientByUid = new Map<string, string>();
  const projectByUid = new Map<string, string>();
  const clientByProjectPageId = new Map<string, string[]>();

  async function paginate(
    dbId: string,
    onPage: (id: string, props: NotionPropertyBag) => void
  ): Promise<void> {
...
/** A Time Log entry that is missing at least one relation, plus its sources. */
export interface TimeLogEntryNeedingRelations {
  /** The Time Log page id. */
  id: string;
 ...
 */
export async function loadTimeLogEntriesNeedingRelations(
  notion: Client
): Promise<TimeLogEntryNeedingRelations[]> {
  const dbId = getTimeLogDatabaseId();
  const filter: QueryDatabaseParams["filter"] = {
    and: [
      {
        or: [
  ...
  const out: TimeLogEntryNeedingRelations[] = [];
  let cursor: string | undefined;
  do {
    const resp = await queryDatabase(notion, dbId, {
      filter: filter,
...
/** Per-relation counts of how many entries had that relation filled in. */
export interface BackfillFilledCounts {
  client: number;
  project: number;
  task: number;
  githubItem: number;
...
/** Outcome of one `backfillTimeLogRelations` pass. */
export interface BackfillResult {
  considered: number;
  updated: number;
  filled: BackfillFilledCounts;
  /** Entries that still lack Client and/or Project after the pass. */
  stillUnlinkedIds: string[];
  /** Candidates skipped because the cap/deadline was hit. */
  remaining: number;
  stoppedEarly: boolean;
  errors: number;
}

/** Bounds and inputs for one `backfillTimeLogRelations` pass. */
export interface BackfillOptions {
  /** Max entries to process this pass (default: unbounded). */
  maxEntries?: number;
  /** `Date.now()` value at which to stop processing (default: unbounded). */
  deadlineMs?: number;
  /** Maps from `loadUniqueIdMaps`, enabling the title-token fallbacks. */
  uidMaps?: UniqueIdMaps;
  /** Resolve and log, but never write. */
  dryRun?: boolean;
}
 ...
 */
export async function backfillTimeLogRelations(
  notion: Client,
  entries: TimeLogEntryNeedingRelations[],
  ...
  for (const entry of entries) {
    ...
    if (!needClient && !needProject && !needTask && !needGithubItem) continue;
    ...
    try {
      ...
      if (
        entry.githubItemIds.length > 0 &&
        (needClient || needProject || needTask)
      ) {
        const gh = await notion.pages.retrieve({
          page_id: entry.githubItemIds[0]!,
        });
        if ("properties" in gh) {
          const p = gh.properties;
          if (needClient) {
            const v = readRelationIds(p, GH_SRC.client);
            if (v.length) resolved.client = v;
          }
          if (needProject) {
            const v = readRelationIds(p, GH_SRC.project);
            if (v.length) resolved.project = v;
          }
          if (needTask) {
            const v = readRelationIds(p, GH_SRC.task);
            if (v.length) resolved.task = v;
          }
      ...
      if (
        ...
        (stillClient || stillProject || needGithubItem)
      ) {
        ...
        if ("properties" in tk) {
          ...
          if (needGithubItem) {
            ...
            if (v.length) resolved.githubItem = v;
          }
        }
      }
      ...
      if (needClient && !resolved.client && opts.uidMaps) {
        ...
        if (projId) {
          ...
          if (c && c.length) resolved.client = c;
        }
      }
      ...
      if (!("properties" in cur)) continue;
      const cp = cur.properties;

      const updates: Record<string, RelationUpdate> = {};
      ...
      if (resolved.project && readRelationIds(cp, TL.project).length === 0) {
        updates[TL.project] = {
          relation: resolved.project.map((id) => ({ id })),
        };
      }
      ...
      if (
        resolved.githubItem &&
        readRelationIds(cp, TL.githubItem).length === 0
      ) {
      ...
      result.updated++;
      if (updates[TL.client]) result.filled.client++;
      if (updates[TL.project]) result.filled.project++;
      if (updates[TL.task]) result.filled.task++;
      if (updates[TL.githubItem]) result.filled.githubItem++;
      console.log(
        TAG,
        `${opts.dryRun ? "[DRY RUN] " : ""}filled ${Object.keys(updates).join(
          ", "
        )} on Time Log ${entry.id}`
      );
    } catch (e) {
      result.errors++;
      console.warn(
        TAG,
        `backfill failed for Time Log ${entry.id}: ${
