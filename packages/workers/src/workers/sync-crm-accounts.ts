...
/**
 * A single row of the COQL projection below. Zoho returns `null` for blank
 * text fields, so every optional CRM field is modelled as `string | null`.
 * `Account_Name` is mandatory on the CRM layout, hence non-nullable.
 */
type CrmAccount = {
  id: string;
  Account_Name: string;
...
/**
 * Notion page property payload. The `properties` map accepted by
 * `pages.create` and `pages.update` is structurally identical, so one alias
 * serves both call sites.
 */
type NotionPageProperties = NonNullable<CreatePageParameters["properties"]>;

export type SyncCrmAccountsInput = {
  max_accounts?: number | null;
  dry_run?: boolean | null;
  only_missing_notion_id?: boolean | null;
};

export type SyncCrmAccountsOutput = {
  success: boolean;
  created: number;
  updated: number;
  crm_patched: number;
 ...
 */
async function findNotionClientByCrmId(
  notion: Client,
  databaseId: string,
  crmAccountId: string
): Promise<string | null> {
...
function buildNotionProperties(account: CrmAccount): NotionPageProperties {
...
export async function executeSyncCrmAccounts(
  input: SyncCrmAccountsInput,
  notion: Client
): Promise<SyncCrmAccountsOutput> {
  ...
  const output: SyncCrmAccountsOutput = {
    success: true,
    created: 0,
    updated: 0,
    crm_patched: 0,
  ...
  try {
    accounts = await zohoCoql<CrmAccount>(query);
  } catch (e) {
    return {
      ...output,
      success: false,
  ...
  for (const account of accounts) {
    ...
    try {
      ...
      const existingPageId = await findNotionClientByCrmId(
        notion,
        databaseId,
        account.id
      );
      ...
      if (!dryRun) {
        try {
          ...
          output.errors.push(
            `CRM patch failed for account ${account.id} (${account.Account_Name}): ${extractErrorMessage(e)}`
          );
        }
      } else {
        output.crm_patched++;
      ...
      output.errors.push(
        `Error processing account ${account.id} (${account.Account_Name}): ${extractErrorMessage(e)}`
      );
    }
  }
