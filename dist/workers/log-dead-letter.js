import { getDeadLettersDatabaseId, extractErrorMessage } from "../shared/notion-client.js";
const VALID_FAILURE_TYPES = ["Missing Digest", "Partial Run", "Failed Run", "Stale Snapshot"];
const VALID_DETECTED_BY = ["Dead Letter Logger", "Morning Briefing", "Manual", "Fleet Ops Agent", "check-agent-staleness"];
export async function executeLogDeadLetter(input, notion) {
    ...
    if (!VALID_FAILURE_TYPES.includes(input.failure_type)) {
        return { success: false, error: `Invalid failure_type: ${input.failure_type}` };
    }
    if (!VALID_DETECTED_BY.includes(input.detected_by)) {
