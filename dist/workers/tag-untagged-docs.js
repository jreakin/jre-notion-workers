import { getDocsDatabaseId, getHomeDocsDatabaseId, extractErrorMessage, queryDatabase } from "../shared/notion-client.js";
import { AGENT_DIGEST_PATTERNS } from "../shared/agent-config.js";
export function inferDocType(title) {
    ...
    if (/Proposal/i.test(title))
        return { type: "Proposal", rule: "title_keyword_proposal" };
    if (/Report|Audit/i.test(title))
        return { type: "Report", rule: "title_keyword_report" };
    if (/Spec|Technical/i.test(title))
        return { type: "Technical Spec", rule: "title_keyword_spec" };
    if (/Meeting|Notes/i.test(title))
        return { type: "Meeting Notes", rule: "title_keyword_meeting" };
    if (/Invoice|Receipt/i.test(title))
        return { type: "Financial", rule: "title_keyword_financial" };
    if (/Contract|Agreement|SOW/i.test(title))
        return { type: "Contract", rule: "title_keyword_contract" };
    // 10. No match
    return { type: null, rule: "no_match" };
}
async function tagFromDatabase(notion, dbId, maxPages, dryRun) {
    const response = await queryDatabase(notion, dbId, {
        filter: { property: "Doc Type", select: { is_empty: true } },
...
export async function executeTagUntaggedDocs(input, notion) {
