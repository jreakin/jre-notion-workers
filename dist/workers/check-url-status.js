...
export async function executeCheckUrlStatus(input) {
        ...
        try {
            ...
            if (lastMod) {
                ...
                if (!isNaN(lastModDate.getTime())) {
                    result.age_hours = Math.round((Date.now() - lastModDate.getTime()) / (1000 * 60 * 60));
                }
            }
    ...
    const isResultOk = (r) => r.reachable && !r.stale && (r.content_match === null || r.content_match === true);
    ...
    const issues = results
        ...
        .map((r) => {
        if (!r.reachable)
            return `${r.label} unreachable`;
        if (r.stale)
            return `${r.label} is stale (${r.age_hours}h)`;
        if (r.content_match === false)
            return `${r.label} content mismatch`;
        return `${r.label} issue`;
    });
    const summary = okCount === results.length
