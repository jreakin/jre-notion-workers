export async function executeLintAgentsFile(input) {
    ...
    findings.push(/<!--\s*version:\s*\d+\.\d+\.\d+\s*-->/.test(content)
    ...
    findings.push(/^#{2,3}\s+Scope\b/m.test(content)
    ...
    findings.push(/Last reviewed:\s*\d{4}-\d{2}-\d{2}/i.test(content)
    ...
    findings.push(/^#{2,3}\s+Reference Documentation\b/m.test(content)
    ...
    // 6. Changelog section
    findings.push(/^#{2,3}\s+Changelog\b/m.test(content)
    ...
    if (isOverlay) {
        findings.push(/[Ee]xtends:\s*.*AGENTS\.md/i.test(content)
