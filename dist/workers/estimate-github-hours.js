...
/* ── PR estimation ──────────────────────────────────────────────────── */
function baseEstimateFromDiff(linesChanged) {
    if (linesChanged <= 50)
        return 0.5;
    if (linesChanged <= 150)
        return 1;
    ...
    if (linesChanged <= 800)
        return 4;
    if (linesChanged <= 1500)
        return 6;
    return 8;
}
function labelMultiplierForPR(labels) {
...
function complexityFactorForPR(changedFiles, files) {
    let factor = 0;
    // File count bonuses
    if (changedFiles > 10) {
        factor += 1;
...
function prConfidence(labels, linesChanged, changedFiles) {
...
async function estimatePR(owner, repo, number, token) {
    // 1. Fetch PR details
    const prResult = await ghFetch(`https://api.github.com/repos/${owner}/${repo}/pulls/${number}`, token);
    if (!prResult.ok)
        return { success: false, error: prResult.error };
    const pr = prResult.data;
    // 2. Fetch per-file diff stats
    const filesResult = await ghFetch(`https://api.github.com/repos/${owner}/${repo}/pulls/${number}/files`, token);
    if (!filesResult.ok)
...
/* ── Issue estimation ───────────────────────────────────────────────── */
function baseEstimateFromIssueLabels(labels) {
    if (labels.includes("documentation"))
        return 0.5;
...
function bodyLengthMultiplier(body) {
...
async function estimateIssue(owner, repo, number, token) {
    const issueResult = await ghFetch(`https://api.github.com/repos/${owner}/${repo}/issues/${number}`, token);
    if (!issueResult.ok)
...
/* ── Entry point ────────────────────────────────────────────────────── */
export async function executeEstimateGitHubHours(input) {
