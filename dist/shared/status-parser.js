const STATUS_PREFIXES = ["Sync Status:", "Snapshot Status:", "Report Status:"];
const EMOJI_MAP = {
    "✅": "complete",
    "⚠️": "partial",
    "❌": "failed",
};
function statusPrefixToType(prefix) {
    if (prefix.startsWith("Sync"))
        return "sync";
    if (prefix.startsWith("Snapshot"))
        return "snapshot";
 ...
 */
export function parseStatusLine(lines) {
    ...
    for (const line of slice) {
        ...
        for (const prefix of STATUS_PREFIXES) {
            if (trimmed.startsWith(prefix)) {
                ...
                let status_value = "complete";
                if (rest.startsWith("⚠️"))
                    status_value = "partial";
                else if (rest.startsWith("❌"))
                    status_value = "failed";
                else if (rest.toLowerCase().includes("stub"))
                    status_value = "stub";
                else if (rest.toLowerCase().includes("full"))
                    status_value = "full_report";
                return {
 ...
 */
export function parseRunTime(lines) {
    const slice = lines.slice(0, 10);
    for (const line of slice) {
        const trimmed = line.trim();
...
export function buildStatusLine(statusType, statusValue) {
    ...
    const label = statusType === "sync"
        ? "Sync Status"
        : statusType === "snapshot"
            ? "Snapshot Status"
            : "Report Status";
    const value = statusType === "sync"
