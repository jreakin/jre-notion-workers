...
**Deployed workers**  
Set secrets via the ntn CLI, or sync from 1Password:

```bash
ntn workers env set NTN_API_TOKEN=...
ntn workers env set DOCS_DATABASE_ID=...
# Full sync (local): export NOTION_API_TOKEN then
bash scripts/sync-notion-worker-secrets.sh
```

GitHub Actions sync: see [docs/GITHUB-SECRETS-SYNC.md](docs/GITHUB-SECRETS-SYNC.md).
